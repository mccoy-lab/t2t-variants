...
### USAGE: ./run_ibdmix_bypop.sh <chr num> <population ID>
### <chr num>: number of chromosome to run on (ex: `21`)
### <population ID>: superpopulation of samples to run on (ex: `EUR`)
###
### Runs ibdmix on GRCh38, using one 1KGP superpopulation as a reference.
...
# if list of samples from superpopulation doesn't exist, make it
mkdir -p vcfs
if ! [[ -f ${POP}_samples.txt  ]]; then
	grep $POP $METADATA | cut -f 1 > vcfs/${POP}_samples.txt
fi
# subset samples from 1KGP NYGC VCF
mkdir -p vcfs/1KGP
$BCFTOOLS view \
	-S vcfs/${POP}_samples.txt \
	--force-samples \
	-v snps \
	-O z \
	-o vcfs/1KGP/chr${i}_NYGC_${POP}.vcf.gz \
	${NYGC_VCFS}/CCDG_14151_B01_GRM_WGS_2020-08-05_chr${i}.*.vcf.gz
$TABIX -p vcf \
